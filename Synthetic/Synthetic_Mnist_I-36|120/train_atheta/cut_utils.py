...
def get_diracs(data, N , device, n_diracs = 1,  sparse = False, flat = False, replace = True, receptive_field = 7, effective_volume_range = 0.1, max_iterations=20, complement = False):
    ...
    if not sparse:
        ...
        return diracmatrix
            ...
            if not is_undirected(data.edge_index):
                data.edge_index,data.edge_attr = to_undirected(data.edge_index, data.edge_attr) # 如果是无向图当中的每一条边，都需要用edge_index 当中的两列来进行表示。比如：[0,1][1,0]表示一条0和1之间的边

            original_batch_index = data.batch
            original_edge_index = data.edge_index
            # original_edge_index = add_remaining_self_loops(data.edge_index, num_nodes = data.batch.shape[0])[0] # 保留自环？
...
#                 batch_prime = torch.cat((batch_prime, dirac_indices))
            ...
            if complement:
                return Batch(batch = batch_index, x = diracmatrix, edge_index = original_edge_index, edge_attr = data.edge_attr,
