...
def rounding(x,batch_node,discrete,adj,row_graph,col_graph,edge_attr,batch_graph,gamma,aaa,retdz,graph,zero_a,loss,test):
    for j in range(len(x[batch_node])):
        ...
        solution_value_0 = (pairwise_square_0.unsqueeze(-1)).sum()/2 # 割集值
        loss_0 = 120./36. * (gamma - solution_value_0) + gamma * torch.exp(-aaa*(eigenvalues_0[2]-eigenvalues_0[1]))
...
def main():
    ...
    for data in test_loader:
        ...
        for graph in range(num_graphs):
            ...
            for j in range(len(xx[batch_node])):
                ...
                loss_0 = 120./36.*(gamma - solution_value_0) + gamma * torch.exp(-aaa*(eigenvalues_0[2]))
                x_test[j] = 1
                ...
                loss_1 = 120./36.*(gamma - solution_value_1) + gamma * torch.exp(-aaa*(eigenvalues_1[2]))
                if loss_1 < loss_0:
                    discrete[batch_node] = x_test.clone()
                    test = eigenvalues_1[2]
                    loss = loss_1
                    zero_a = len(xx[batch_node])-torch.count_nonzero(eigenvalues_1)
                else:
                    x_test[j] = 0
                    discrete[batch_node] = x_test.clone()
                    test = eigenvalues_0[2]
                    loss = loss_0
                    zero_a = len(xx[batch_node])-torch.count_nonzero(eigenvalues_0)
            
            if torch.round(loss*10)/10 >= torch.round(retdz["losses histogram"][0][graph]*10)/10: 
                label_abc = zero_a
                label_abc_num = 0
                loss,x_test,test,zero_a = rounding(xx,batch_node,discrete,adj[graph],row_graph,col_graph,edge_attr,batch_graph,gamma,aaa,retdz,graph,zero_a,loss,test)
    ...
    mean_sum = mean_sum/float(len(test_dataset)-not_connected)
